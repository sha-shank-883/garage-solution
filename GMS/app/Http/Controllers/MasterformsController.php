...
class MasterformsController extends Controller
{
	...
	public function addUser(Request $request, $id=null, $view = null)
	{
		...
		$getRoles += Role::where('is_active', '1')->where('id','!=' ,1)->pluck('role_name', 'id')->toArray();
...
// $result = $result->toArray();
	...
	public function userList()
	{
		...
		$userDetail= DB::table('users');
			$userDetail->leftJoin('user_details','users.id','=','user_details.users_id');
            $userDetail->leftJoin('roles','roles.id','=','users.role_id');
              $userDetail->select('users.*','user_details.*','users.id as UserId','roles.*');
 $userDetail=$userDetail->get();
 $users=json_decode(json_encode($userDetail), true);
		//$users = User::where('role_id', '!=', '1')->orderBy('id', 'desc')->with('role')->with('user_detail')->get();
		//$laQuery = DB::getQueryLog();
		// print_r($laQuery);echo "<br/>";
		// print_r($users);
	//	DB::disableQueryLog();
		//exit();
		$customFields['basic'] = array(
		...
		return view('admin.user-list', ['otherLinks' => array('link' => url('/').'/employee', 'text' => 'Add Employee'), 'pageTitle' => 'Employee List', 'tableData' => $users,'loopInit' => '1']);
	}
	...
	{
		$sql="select count(id) as totalPendingJob from workshops where is_complete=0";
		...
		return view('AutoCare.dashboard',$userData);
	}
