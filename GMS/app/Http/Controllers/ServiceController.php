...
class ServiceController extends Controller
{
    ...
    public function save(Request $request, $id = null)
    {
        ...
        $viewData['header_link'] =  HeaderLink::where("menu_id",'5')->select("link_title","link_name")->orderBy('id','desc')->get();
			...
			$viewData['model_list'] = Modal::with('brand')->orderBy('created_at', 'desc')->paginate(5);;
			$viewData['service_type_list'] = ServiceType::orderBy('created_at', 'desc')->paginate(5);;
			$viewData['service_name_list'] = ServiceName::orderBy('created_at', 'desc')->paginate(5);;
     ...
     //   	if(isset($id) && $id != null ){
    	...
    	// 	return view('AutoCare.service.add', $viewData)->with($getFormAutoFillup);
    	// }
    	// // else if((!isset($id) && $id == null) && !$request->isMethod('post') )
    	// // {
    	// // 	return view('AutoCare.service.add', $viewData);
    	// // }
    	// else {
	    	...
	    	// 		if ($request->isMethod('post')){
			   //  		$serviceManame=	 request()->except(['_token']);
					 ...
					 //     	$request->session()->flash('message.content', 'Service updated Successfully!');
						// }
				  //   }
				  //    return redirect('/AutoCare/service/add/'.$request->id);
	    	// 	}
	    	// 	else
	    	// 	{
     ...
     public function view(Request $request)
    {
    	...
    	if($request->isMethod('post'))
    	{
            ...
            $Service->leftJoin('modals','modals.id','=','services.model_name');
            $Service->leftJoin('brands','brands.id','=','services.brand_name'); 
            $Service->leftJoin('service_types','service_types.id','=','services.service_type');
   ...
   //     		$Service= $Service->get();
		    ...
		    return view('AutoCare.service.search', $viewData);
    ...
    public function trashedList()
    {
         ...
         return view('AutoCare.service.delete', compact('TrashedService', 'TrashedService')); 
    }
